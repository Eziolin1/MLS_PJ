...
#         else:
    ...
    def _target_update_inps(self, inps, grad, step_size):
        unnorm_inps = self._mul_std_add_mean(inps.clone().detach())
        unnorm_inps = unnorm_inps - step_size * grad.sign()
        unnorm_inps = torch.clamp(unnorm_inps, min=0, max=1).detach()
        adv_inps = self._sub_mean_div_std(unnorm_inps)
    ...
    def _target_update_perts(self, perts, grad, step_size):
        perts = perts - step_size * grad.sign()
        perts = torch.clamp(perts, -self.epsilon, self.epsilon)
        return perts
...
class OurAlgorithm(BaseAttack):
    def __init__(self, model_name, target='0', ablation_study='1,1,1', sample_num_batches=130, lamb=0.1, steps=10, epsilon=16/255):
    ...
    def forward(self, inps, labels):
        ...
        for i in range(self.steps):
            ...
            if self.target:
                perts.data = self._target_update_perts(perts.data, grad, self.step_size)
            else:
    ...
    def forward(self, inps, labels):
        ...
        for i in range(self.steps):
            if self.ablation_study[0] == '1':
                print ('Using Pathes')
                add_perturbation = self._generate_samples_for_interactions(perts, i)
                outputs = self.model((self._sub_mean_div_std(unnorm_div_inps + add_perturbation)))
            else:
                print ('Not Using Pathes')
                outputs = self.model((self._sub_mean_div_std(unnorm_div_inps + perts)))
...
class OurAlgorithm_TI(BaseAttack):
    def __init__(self, model_name, target=False, ablation_study='1,1,1', sample_num_batches=130, lamb=0.1, steps=10, epsilon=16/255):
        ...
        self.stack_kernel = np.expand_dims(self.kernel, 0)
    ...
    def forward(self, inps, labels):
        ...
        for i in range(self.steps):
            ...
            grad_1 = grad[:, 0, :, :].unsqueeze(1)
            grad_2 = grad[:, 1, :, :].unsqueeze(1)
            grad_3 = grad[:, 2, :, :].unsqueeze(1)

            conv = nn.Conv2d(1, 1, (1, 1, 15, 15), padding='same', bias=False)
            conv.weight.data = torch.from_numpy(self.stack_kernel).cuda()
            grad_1 = conv(grad_1).data
            grad_2 = conv(grad_2).data
            grad_3 = conv(grad_3).data
            grad[:, 0, :, :] = grad_1
            grad[:, 1, :, :] = grad_2
            grad[:, 2, :, :] = grad_3
...
class OurAlgorithm_TI_MI(BaseAttack):
    def __init__(self, model_name, target=False, ablation_study='1,1,1', sample_num_batches=130, lamb=0.1, steps=10, epsilon=16/255, decay=1.0):
        super(OurAlgorithm_TI_MI, self).__init__('OurAlgorithm_TI_MI', model_name, target)
        self.epsilon = epsilon
        self.steps = steps
        self.step_size = self.epsilon/self.steps
        self.decay = decay
...
class OurAlgorithm_SIM(BaseAttack):
    def __init__(self, model_name, target='0', ablation_study='1,1,1', sample_num_batches=130, lamb=0.1, steps=10, epsilon=16/255, decay=1.0):
        super(OurAlgorithm_SIM, self).__init__('OurAlgorithm_SIM', model_name, target)
        self.epsilon = epsilon
        self.steps = steps
        self.step_size = self.epsilon/self.steps
        self.target = target
    ...
    def forward(self, inps, labels):
        ...
        for i in range(self.steps):
            ...
            for j in range(5):
                inps.requires_grad_()
                grad1 = 0
                S_unnorm_inps_nes = unnorm_inps_nes / pow(2, j)
                outputs = self.model((self._sub_mean_div_std(S_unnorm_inps_nes + add_perturbation)))
                cost11 = self.loss_flag * loss(outputs, labels).cuda()
                cost21 = torch.norm(perts)
                cost3 = cost11 + self.lamb * cost21
                grad1 = torch.autograd.grad(cost3, inps, retain_graph=True)[0]
                if j == 0:
                    g = torch.zeros_like(grad1).cuda()
                g = g + grad1
            ...
            cost.backward()
            grad = perts.grad.data
            g = 1 / 5 * g / torch.mean(torch.abs(g), dim=[1,2,3], keepdim=True)
            grad = self.decay * grad + g
            if self.target:
                perts.data = self._target_update_perts(perts.data, grad, self.step_size)
            else:
                perts.data = self._update_perts(perts.data, grad, self.step_size)
            perts.data = torch.clamp(unnorm_inps.data + perts.data, 0.0, 1.0) - unnorm_inps.data
            unnorm_inps_nes = unnorm_inps + self.decay * self.step_size * grad
            perts.grad.data.zero_()
        return (self._sub_mean_div_std(unnorm_inps+perts.data)).detach(), None
    
class OurAlgorithm_SGM_SIM(BaseAttack):
    def __init__(self, model_name, target=False, sgm_control='1,0', sample_num_batches=130, lamb=0.1, steps=10, epsilon=16/255, decay=1.0):
        super(OurAlgorithm_SGM_SIM, self).__init__('OurAlgorithm_SGM_SIM', model_name, target)
        self.epsilon = epsilon
        self.steps = steps
        self.step_size = self.epsilon/self.steps
        self.sgm_control = sgm_control.split(',')
